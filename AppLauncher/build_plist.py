...
1. Import `App Launcher.alfredworkflow`, then press any Hyper+key (or run
   `app` in Alfred).
2. On first use the registry is imported automatically from the old
   "application shortcuts" workflow if it is still installed (Hyper+1/2/3,
   Hyper+B/C/D/F/M/N/W/Z → your apps, plus ⌥⇧S → Easydict).
