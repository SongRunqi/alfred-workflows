...
def cmd_launch(key: str, query: str = "") -> None:
    ...
    if not entry:
        if notify_unassigned_enabled():
            notify(f"Hyper+{key.upper()} 未绑定应用\n在 Alfred 输入: app add {key} <应用名>")
        return
...
def cmd_add(args: list) -> None:
    ...
    if key in reg_apps:
        notify(f"Hyper+{key.upper()} 已被 {reg_apps[key].get('name')} 占用\n先 app rm {key}")
        return
    path = resolve_app(name)
    if not path:
        notify(f"找不到应用: {name}")
    ...
    reg_apps[key] = entry
    save_registry(reg)
    notify(f"✓ Hyper+{key.upper()} → {Path(path).stem}")
...
def cmd_remove(target: str) -> None:
    ...
    del reg_apps[key]
    save_registry(reg)
    notify(f"✕ 已移除 Hyper+{key.upper()} → {name}")
...
def ensure_registry() -> bool:
    """Auto-migrate on first run. Returns True if a fresh registry was created."""
    if registry_path().exists():
        return False
    migrated = migrate(force=True)
    if migrated:
        names = ", ".join(f"{k.upper()}→{n}" for k, n in migrated)
        notify(f"已从旧工作流导入 {len(migrated)} 个快捷键映射\n{names}")
    return True
